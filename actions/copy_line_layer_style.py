"""
Copy Line Layer Style Action for Right-click Utilities and Shortcuts Hub

Allows copying the style (symbology, labeling, effects) from the clicked line layer
to other line layers in the project. Shows a dialog where users can select which
line layers to copy the style to.
"""
...
class CopyLineLayerStyleDialog(QDialog):
    """Dialog for selecting target line layers to copy style to."""
    ...
    def __init__(self, source_layer, target_layers, parent=None):
        ...
        self.setWindowTitle("Copy Line Layer Style")
    ...
    def init_ui(self):
        ...
        # Instructions
        instructions = QLabel("Select line layers to copy the style to:")
...
class CopyLineLayerStyleAction(BaseAction):
    """Action to copy style from one line layer to other line layers."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "copy_line_layer_style"
        self.name = "Copy Line Layer Style"
        self.category = "Styling"
        self.description = (
            "Copy the style (symbology, labeling, effects) from this line layer "
            "to other line layers in the project. A dialog will show all available "
            "line layers where you can select which ones to apply the style to."
        )
        self.enabled = True
    ...
    def execute(self, context):
        """Execute the copy line layer style action."""
        try:
        ...
        if not detected_features:
            self.show_error("Error", "No line layer found at this location")
            return
        ...
        if source_layer.geometryType() != QgsWkbTypes.LineGeometry:
            self.show_error("Error", "Source layer is not a line layer")
            return
        
        # Get all other line layers in the project
        target_layers = []
        ...
        if not target_layers:
            self.show_info("No Layers", "No other line layers found in the project")
            self.record_informational(
                description=f"Attempted to copy style from line layer '{source_layer.name()}' but no other line layers were available"
            )
            return
        
        # Show dialog to select target layers
        dialog = CopyLineLayerStyleDialog(source_layer, target_layers, None)
        ...
        try:
            ...
            if confirm_action:
                if errors:
                    ...
                    self.show_info(
                        "Copy Style Complete",
                        f"Successfully copied style from '{source_layer.name()}' to {copied_count} line layer(s)"
                    )
            
            # Record to history
            self.record_informational(
                description=f"Copied style from line layer '{source_layer.name()}' to {copied_count} line layer(s)",
                meta={
...
copy_line_layer_style_action = CopyLineLayerStyleAction()
