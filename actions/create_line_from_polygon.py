...
class CreateLineFromPolygonAction(BaseAction):
    ...
    def _create_line_layer(self, line_geometry, layer_name, source_crs, include_metadata, source_feature_id, source_layer_name, output_file_path=None, file_format=None, create_permanent=False):
        ...
        try:
            ...
            created_fid = None
            try:
                for f in file_layer.getFeatures():
                    created_fid = f.id()
                    break
            except Exception:
                created_fid = None

            return file_layer, created_fid
    ...
    def execute(self, context):
        ...
        try:
            ...
            # Record history entry (CREATE_LAYER) so this action can be undone/redone
            try:
                # get created feature if possible
                ...
                hm.record(
                    ...
                    description=f"Created line from polygon {feature.id()} in layer '{line_layer.name()}'",
