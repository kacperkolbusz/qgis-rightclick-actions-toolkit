"""
Cleanup NULL Fields - Point Layer Action for Right-click Utilities and Shortcuts Hub

Analyzes the attribute table of a point layer and removes fields where a configurable
...
class CleanupNullFieldsPointLayerAction(BaseAction):
    """Action to remove high-NULL fields from point layer attribute tables."""

    def __init__(self):
        """Initialize the action with metadata and configuration."""
        super().__init__()

        self.action_id = 'cleanup_null_fields_point_layer'
        self.name = 'Clean Up NULL Fields'
        self.category = 'Editing'
        self.description = (
            'Analyzes the attribute table of a point layer and deletes fields where '
    ...
    def execute(self, context):
        """Execute the NULL field cleanup action on a point layer."""
        ...
        self.record_to_history(
            description=(
                ...
                f"point layer '{layer.name()}' (threshold: {null_threshold:.0f}%)"
            ),
            ...
            for field_data in deleted_fields:
                ...
                for fid_str, value in values_map.items():
                    try:
                        ...
                        pass  # Skip individual value errors; continue restoring others
    ...
    def _create_field_backup(layer: QgsVectorLayer, field_names: list) -> list:
        ...
        for field_name in field_names:
            ...
            field_entry = {
                'name': field.name(),
                'type': int(field.type()),          # QMetaType.Type integer value
                'type_name': field.typeName(),
                'length': field.length(),
                'precision': field.precision(),
                'comment': field.comment(),
                'alias': field.alias(),
                'values': {},   # str(fid) -> value (JSON-safe keys)
            }

            # Read every feature's value for this field
            for feature in layer.getFeatures():
                ...
                if (val is None
                    ...
                    # Attempt generic conversion; fall back to str
                    try:
...
cleanup_null_fields_point_layer = CleanupNullFieldsPointLayerAction()
