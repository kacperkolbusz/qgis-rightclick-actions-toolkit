    ...
    def execute(self, context):
        ...
        try:
            ...
            if add_to_project:
                ...
                # Record history: creation of the new area layer (store style + labeling to allow exact redo)
                try:
                    ...
                    created_features = list(area_layer.getFeatures())
                    feat_backups = []
                    for f in created_features:
                        fb = hm.create_feature_backup(f, area_layer, include_geometry=True, include_attributes=True)
                        feat_backups.append(fb)

                    # Capture renderer info and try to save full style where possible
                    renderer_info = None
                    try:
                        renderer = area_layer.renderer() if hasattr(area_layer, 'renderer') else None
                        sym_props = None
                        try:
                        ...
                        style_str = None
                        try:
                            if hasattr(area_layer, 'saveStyleToString'):
                                try:
                                    style_str = area_layer.saveStyleToString()
                                except Exception:
                                    style_str = None
                        except Exception:
                            style_str = None
                    ...
                    labeling_info = None
                    try:
                        labeling_mgr = area_layer.labeling() if hasattr(area_layer, 'labeling') else None
                        if labeling_mgr is not None and area_layer.labelsEnabled():
                            try:
                    ...
                    layer_desc = hm.create_layer_descriptor(area_layer)

                    # Layer definition to allow redo of the layer (structure + backups)
                    layer_def = {
                        'layer_name': area_layer.name(),
                        'crs': area_layer.crs().authid() if area_layer.crs().isValid() else '',
                        'geometry_type': QgsWkbTypes.displayString(area_layer.wkbType()),
                        'fields': [],
                        'features': feat_backups,
                        'labeling': labeling_info,
                        'renderer': renderer_info,
                    }
                    for fld in area_layer.fields():
                        try:
                    ...
                    hm.record(
                        ...
                        description=f"Created area layer '{area_layer.name()}' for feature {feature_id}",
