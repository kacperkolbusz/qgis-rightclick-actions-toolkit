        ...
        try:
            ...
            try:
                ...
                self.record_informational(
                    description=f"Zoomed to point feature {feature.id()} on layer '{layer.name()}'.",
                    meta={'layer': layer_desc, 'feature_id': int(feature.id()), 'point': {'x': float(point.x()), 'y': float(point.y())}, 'view': view_meta}
                )
            except Exception:
                pass
