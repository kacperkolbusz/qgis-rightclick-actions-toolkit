...
class AttributeEditDialog(QDialog):
    ...
    def __init__(self, action, feature, layer, parent=None, show_success_messages=True, show_error_messages=True, show_warning_messages=True, confirm_save=False, auto_commit=True):
        ...
        # Enter edit mode via BaseAction helper
        try:
            res = self.action.handle_edit_mode(self.layer, "attribute edit")
            if res and res[0] is not None:
                self._was_in_edit_mode, self.edit_mode_entered = res
            else:
                # Could not enter edit mode, dialog should not proceed
                if self.show_error_messages:
                    QMessageBox.warning(self, "Edit Mode Error", "Could not enter edit mode for this layer.")
                self.reject()
        except Exception:
    ...
    def enter_edit_mode(self):
        ...
        # Fallback local behavior if action helper not available
    ...
    def save_changes(self):
        ...
        try:
            ...
            for i, field in enumerate(fields):
                ...
                # old attributes map
                old_attrs_map[field.name()] = self.original_values.get(field.name())
            
            # Optionally confirm save
            if self.confirm_save:
                if QMessageBox.question(self, "Confirm Save", "Save attribute changes?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) != QMessageBox.Yes:
                    return
    ...
    def cancel_changes(self):
        ...
        self.layer.rollBack()
        self.reject()
        # Rollback using action helper if available
        try:
            if hasattr(self, 'action') and self.action:
                self.action.rollback_changes(self.layer)
            else:
