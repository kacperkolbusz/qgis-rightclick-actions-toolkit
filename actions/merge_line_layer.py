"""
Merge Line Layer Action for Right-click Utilities and Shortcuts Hub

Merges multiple line layers together, combining all features and attributes.
...
class LayerSelectionDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = QLabel(f"Select line layers to merge with '{self.initial_layer_name}':")
...
class MergeLineLayerAction(BaseAction):
    """Action to merge multiple line layers together."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "merge_line_layer"
        self.name = "Merge Line Layer"
        self.category = "Editing"
        self.description = "Merge multiple line layers together, combining all features and attributes. Checks for attribute compatibility and allows merging even if attribute columns differ. Optionally deletes source layers after merge."
        self.enabled = True
        ...
        # Undo state
        self._last_created_layer_descriptor = None
        self._last_deleted_source_descriptors = []
    ...
    def _get_line_layers(self, exclude_layer):
        """
        Get all line layers from the project, excluding the specified layer.
        
        Args:
            exclude_layer: Layer to exclude from the list
            
        Returns:
            list: List of line layers
        """
    ...
    def execute(self, context):
        """Execute the merge line layer action."""
        try:
            ...
            # Check if layer has line features
            if layer1.geometryType() != QgsWkbTypes.LineGeometry:
                self.show_error("Error", "This action only works with line layers")
                return
            
            # Get all other line layers
            other_layers = self._get_line_layers(layer1)
            
            if not other_layers:
                self.show_error("Error", "No other line layers found in the project to merge with")
                return
            ...
            # Prepare history descriptors BEFORE deletion
            source_descriptors = [self.create_layer_descriptor(layer) for layer, _ in layers_with_sources]

            # Store created layer descriptor for history (will be updated after we have merged_layer)
            # We'll populate _last_created_layer_descriptor later once merged_layer is available
            ...
            # Record action to history (undo support)
            try:
                created_desc = self.create_layer_descriptor(merged_layer)
                self._last_created_layer_descriptor = created_desc

                # Include source descriptors - these were captured before deletion
                self._last_deleted_source_descriptors = source_descriptors if delete_original else []

                # Build composite sub-operations list
                sub_ops = []

                # Sub-op: created merged layer (create_layer)
                # Build a full layer definition (fields + feature backups) so redo can recreate the layer
                layer_def = {
                    'layer_name': created_desc.get('layer_name'),
                    'crs': created_desc.get('crs'),
                    'geometry_type': created_desc.get('geometry_type'),
                    'fields': [],
                    'features': []
                }
                ...
                try:
                    # Fields
                    for f in merged_layer.fields():
                        try:
                            qmeta = f.type()
                        except Exception:
                            qmeta = None
                        layer_def['fields'].append({'name': f.name(), 'qmeta_type': qmeta})

                    # Feature backups
                    for feat in merged_layer.getFeatures():
                        try:
                            fb = self.create_feature_backup(feat, merged_layer)
                            layer_def['features'].append(fb)
                        except Exception:
                            continue
                except Exception:
                    # If anything goes wrong, fall back to minimal definition
                    layer_def = {
                        'layer_name': created_desc.get('layer_name'),
                        'crs': created_desc.get('crs'),
                        'geometry_type': created_desc.get('geometry_type')
                ...
                create_layer_sub = {
                    ...
                    # Provide a redo_payload mirror so CompositeHandler has stable data
                    'redo_payload': {
                        'layer_definition': layer_def
                    },
                    'layers': [created_desc],
                ...
                sub_ops.append(create_layer_sub)

                # NOTE: We do NOT include delete_layer sub-ops unless a full backup is created.
                # The DeleteLayerHandler in this toolkit requires a full 'layer_backup' which
                # is not currently produced by this action. Skipping ensures composite undo
                # still works to remove the merged layer.

                # Build layers list for history entry: include created layer descriptor
                history_layers = [created_desc]

                # Record history entry using CREATE_LAYER undo type (store full layer definition)
                try:
                    from ..history_manager import HistoryManager

                    layer_def_payload = {'layer_definition': layer_def}

                    self.record_to_history(
                        description=f"Merged {len(all_layers_to_merge)} layers into '{merged_layer_name}'",
                        undo_type=HistoryManager.UNDO_TYPE_CREATE_LAYER,
                        can_undo=True,
                        undo_payload=layer_def_payload,
                        layers=history_layers,
                        features=layer_def.get('features', []),
                        meta={'created_permanent': create_permanent, 'deleted_original': delete_original}
                    )
                except Exception:
                    # Fallback to string type if HistoryManager import fails
                    try:
                        self.record_to_history(
                            description=f"Merged {len(all_layers_to_merge)} layers into '{merged_layer_name}'",
                            undo_type='create_layer',
                ...
                # Don't block the rest of the flow on history recording
                pass
    ...
    def supports_undo(self) -> bool:
        """This action supports undo (composite: created layer and optional deletions)."""
        return True
    ...
    def apply_undo(self, payload: dict) -> tuple:
        """Undo by removing the created merged layer. Restoring deleted source files is not automatic.

        The payload is expected to contain keys:
          - 'created_layer': layer descriptor for the merged layer
          - 'deleted_sources': list of descriptors for any source layers that were deleted
        """
        try:
            from qgis.core import QgsProject, QgsVectorLayer

            created = payload.get('created_layer') or {}
            created_id = created.get('layer_id')

            # Remove merged layer if present
            if created_id:
                proj = QgsProject.instance()
                layer = proj.mapLayer(created_id)
                if layer:
                    proj.removeMapLayer(created_id)

            # If source layers were deleted, we cannot reliably restore files here.
            deleted_sources = payload.get('deleted_sources', []) or []
            if deleted_sources:
                message = (
                    "Merged layer removed. Source layers were deleted during the merge and "
                    "cannot be automatically restored by this undo handler. Please restore files manually if needed."
                )
                return True, message

            return True, "Merged layer removed"
        except Exception as e:
            return False, f"Undo failed: {str(e)}"

    def apply_redo(self, payload: dict) -> tuple:
        """Redo the merge by re-adding the merged layer if persisted to disk.

        If the merged layer was saved to disk (data_source in descriptor), attempt to reload it.
        Memory layers cannot be re-created by this redo handler.
        """
        try:
            from qgis.core import QgsProject, QgsVectorLayer

            created = payload.get('created_layer') or {}
            data_source = created.get('data_source')
            is_temporary = created.get('is_temporary', True)

            if data_source and not is_temporary and os.path.exists(data_source):
                # Try to reload
                layer = QgsVectorLayer(data_source, created.get('layer_name', 'merged'), 'ogr')
                if layer and layer.isValid():
                    QgsProject.instance().addMapLayer(layer)
                    return True, 'Merged layer reloaded from disk'
                else:
                    return False, 'Failed to reload merged layer from disk'

            return False, 'Redo not supported for memory layers or missing data source'
        except Exception as e:
...
merge_line_layer_action = MergeLineLayerAction()
