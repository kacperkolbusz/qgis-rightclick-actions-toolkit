"""
Generalize Line Layer Action for Right-click Utilities and Shortcuts Hub

Generalizes all line features in the selected layer using the Douglas-Peucker algorithm.
...
from qgis.PyQt.QtWidgets import (
    ...
    QFormLayout, QDoubleSpinBox, QCheckBox, QGroupBox, QComboBox
)
from qgis.PyQt.QtCore import QVariant, QMetaType


class GeneralizeLineLayerDialog(QDialog):
    """Dialog for configuring layer-level line generalization."""
    ...
    def __init__(self, parent=None, default_tolerance=1.0,
        ...
        self.setWindowTitle("Generalize Line Layer")
        self.setModal(True)
        self.resize(420, 320)
        ...
        tolerance_help = QLabel(
            ...
            "Points within this distance from the simplified line are removed."
        )
...
class GeneralizeLineLayerAction(BaseAction):
    """
    Action to generalize all line features in a layer using the Douglas-Peucker algorithm.

    This action simplifies every line feature in the layer by reducing the number of
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "generalize_line_layer"
        self.name = "Generalize Line Layer"
        self.category = "Editing"
        self.description = (
            "Generalize all line features in the layer using the Douglas-Peucker algorithm. "
            "Reduces the number of vertices across all features while preserving overall shapes. "
            "Supports processing only selected features and creating a generalized copy layer."
        )
        self.enabled = True
    ...
    def execute(self, context):
        """Execute the generalize line layer action."""
        # --- Read settings ---
        try:
        ...
        if layer is None:
            self.show_error("Error", "No line layer found")
            return
        ...
        dialog = GeneralizeLineLayerDialog(
            None,
        ...
        if create_copy:
            ...
            new_layer = QgsVectorLayer(
                f"MultiLineString?crs={layer_crs}",
        ...
        # =====================================================================
            ...
            if handle_edit_mode:
                edit_result = self.handle_edit_mode(layer, "line layer generalization")
            ...
            try:
                for feature in features_to_process:
                    ...
                    # Capture old geometry BEFORE modifying
                    try:
                        old_geom_data = {
                    ...
                    try:
                        new_geom_data = {
                            'wkb_base64': base64.b64encode(generalized_geom.asWkb()).decode('utf-8')
                        }
                    except Exception:
                        new_geom_data = None
                    ...
                    # Build payload dict in the exact format the handler expects
                    feat_payload = {'fid': feature.id()}
                    if old_geom_data:
                        feat_payload['old_geometry'] = old_geom_data
                    if new_geom_data:
                        feat_payload['new_geometry'] = new_geom_data
                    feature_payloads.append(feat_payload)

                if auto_commit and handle_edit_mode:
                    if not self.commit_changes(layer, "line layer generalization"):
                        return
...
generalize_line_layer = GeneralizeLineLayerAction()
