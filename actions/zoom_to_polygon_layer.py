"""
Zoom to Polygon Layer Action for Right-click Utilities and Shortcuts Hub

Zooms the map to show all polygons on the selected polygon layer with buffer.
Works on polygon layers and operates silently without popup windows.
"""
...
class ZoomToPolygonLayerAction(BaseAction):
    """Action to zoom to show all polygons on a polygon layer with buffer."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "zoom_to_polygon_layer"
        self.name = "Zoom to Polygon Layer"
        self.category = "Navigation"
        self.description = "Zoom the map to show all polygons on the selected polygon layer with buffer. Calculates layer extent and adds padding for better visibility. Works on polygon and multipolygon layers."
        self.enabled = True
    ...
    def execute(self, context):
        """
        Execute the zoom to polygon layer action.
        ...
        try:
            ...
            if show_errors:
                self.show_error("Error", f"Failed to zoom to polygon layer: {str(e)}")
            # Silently handle any errors if not showing errors


# REQUIRED: Create global instance for automatic discovery
zoom_to_polygon_layer_action = ZoomToPolygonLayerAction()
