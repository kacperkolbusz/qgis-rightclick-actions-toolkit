"""
Zoom to Point Layer Action for Right-click Utilities and Shortcuts Hub

Zooms the map to show all points on the selected point layer with 10% buffer.
Works on point layers and operates silently without popup windows.
"""
...
class ZoomToPointLayerAction(BaseAction):
    """Action to zoom to show all points on a point layer with buffer."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "zoom_to_point_layer"
        self.name = "Zoom to Point Layer"
        self.category = "Navigation"
        self.description = "Zoom the map to show all points on the selected point layer with 10% buffer. Calculates layer extent and adds padding for better visibility. Works on point and multipoint layers."
        self.enabled = True
    ...
    def get_settings_schema(self):
        ...
        return {
            ...
            'show_error_messages': {
                ...
                'description': 'Display error messages if zoom operation fails (normally silent)',
            },
        }
    ...
    def execute(self, context):
        """
        Execute the zoom to point layer action.
        ...
        try:
            ...
            show_errors = bool(self.get_setting('show_error_messages', False))
        ...
        try:
            ...
            if canvas_crs != layer_crs:
                # Create coordinate transform from layer CRS to canvas CRS
                ...
                try:
                    ...
                    if show_errors:
                        self.show_error("Error", f"Failed to transform extent from {layer_crs.description()} to {canvas_crs.description()}: {str(transform_error)}")
            ...
            try:
                ...
                try:
                    ...
                    view_meta['center'] = {'x': center.x(), 'y': center.y()}
                except Exception:
                    view_meta = None
                ...
                try:
                    ...
                    layer_desc = {'layer_name': getattr(layer, 'name', lambda: 'unknown')()}

                self.record_informational(
                    description=f"Zoomed to layer '{layer.name()}'.",
                    meta={'layer': layer_desc, 'view': view_meta}
                )
            except Exception:
                pass
            ...
            if show_errors:
                self.show_error("Error", f"Failed to zoom to point layer: {str(e)}")
            # Silently handle any errors if not showing errors


# REQUIRED: Create global instance for automatic discovery
zoom_to_point_layer_action = ZoomToPointLayerAction()
