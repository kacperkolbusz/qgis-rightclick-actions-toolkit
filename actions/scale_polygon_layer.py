"""
Scale Polygon Layer Action for Right-click Utilities and Shortcuts Hub

Scales all polygon features in the selected polygon layer by a specified scale factor.
Each polygon is scaled around its own centroid.
"""
...
class ScalePolygonLayerDialog(QDialog):
    ...
    def __init__(self, parent=None, default_scale=1.0, feature_count=None, default_mode='individual', default_scale_objects=True):
        ...
        self.setWindowTitle("Scale Polygon Layer")
        self.setModal(True)
        self.resize(400, 250)
        ...
        self.individual_radio.setToolTip("Each polygon scales around its own centroid")
        ...
        self.scale_objects_checkbox.setToolTip("When scaling the layer, also scale each polygon's geometry around its own centroid")
...
class ScalePolygonLayerAction(BaseAction):
    """
    Action to scale all polygon features in a polygon layer by a specified scale factor.
    
    This action scales all polygon features in the selected layer around their own
    centroid by the scale factor specified by the user. Scale factor > 1.0 makes
    polygons larger, scale factor < 1.0 makes them smaller.
    """
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "scale_polygon_layer"
        self.name = "Scale Polygon Layer"
        self.category = "Editing"
        self.description = "Scale all polygon features in the selected polygon layer by a specified scale factor. Each polygon is scaled around its own centroid. User inputs scale factor (1.0 = original size, >1.0 = larger, <1.0 = smaller)."
        self.enabled = True
    ...
    def get_settings_schema(self):
        ...
        return {
            ...
            'confirm_before_scale': {
                ...
                'description': 'Show confirmation dialog before scaling all polygons in the layer',
            },
    ...
    def _scale_polygon_geometry(self, geometry, scale_factor, center_point=None):
        """
        Scale a polygon geometry around a center point.
        
        Args:
            geometry (QgsGeometry): Polygon geometry to scale
            ...
            try:
                ...
                for ring in polygon:
                    ...
                    for vertex in ring:
                        ...
                        scaled_ring.append(QgsPointXY(new_x, new_y))
                    
                    scaled_polygon.append(scaled_ring)
                
                # Create new geometry from scaled vertices
                return QgsGeometry.fromPolygonXY(scaled_polygon)
            except (TypeError, AttributeError):
                # Handle multipolygon geometry
                try:
                    multi_polygon = geometry.asMultiPolygon()
                    scaled_multi_polygon = []
                    ...
                    for polygon_part in multi_polygon:
                        ...
                        for ring in polygon_part:
                            ...
                            scaled_polygon_part.append(scaled_ring)
                        
                        scaled_multi_polygon.append(scaled_polygon_part)
                    
                    # Create new geometry from scaled vertices
                    return QgsGeometry.fromMultiPolygonXY(scaled_multi_polygon)
                except (TypeError, AttributeError):
                    return None
        except Exception:
            return None
    ...
    def execute(self, context):
        """
        Execute the scale polygon layer action.
        ...
        # Show input dialog
        dialog = ScalePolygonLayerDialog(
            None,
        ...
        if confirm_before_scale:
            if scaling_mode == 'individual':
                confirmation_message = f"Scale all {feature_count} polygon features in layer '{layer.name()}' by {scale_factor:.2f}x?\n\n"
                confirmation_message += "Each polygon will be scaled around its own centroid."
            else:
            ...
            if not self.confirm_action("Scale Polygon Layer", confirmation_message):
                return
        ...
        if handle_edit_mode:
            edit_result = self.handle_edit_mode(layer, "polygon layer scaling")
        ...
        try:
            ...
            for feature in layer.getFeatures():
                ...
                if scaling_mode == 'individual':
                    ...
                    scaled_geometry = self._scale_polygon_geometry(geometry, scale_factor)
                else:
                    # Scale around layer center
                    scaled_geometry = self._scale_polygon_geometry(geometry, scale_factor, layer_center)
                    
                    # If also scaling object sizes, scale again around feature's own centroid
                    if scale_objects:
                        scaled_geometry = self._scale_polygon_geometry(scaled_geometry, scale_factor)
            ...
            if auto_commit and handle_edit_mode:
                if not self.commit_changes(layer, "polygon layer scaling"):
                    return
            ...
            if show_success:
                success_message = f"Polygon layer scaled successfully by {scale_factor:.2f}x"
            ...
            self.show_error("Error", f"Failed to scale polygon layer: {str(e)}")
            if rollback_on_error and handle_edit_mode:
                self.rollback_changes(layer)
...
scale_polygon_layer = ScalePolygonLayerAction()
