"""
Split Line Layer by Attribute Action for Right-click Utilities and Shortcuts Hub

Splits a line layer into multiple layers based on a selected attribute field.
...
class SplitLineLayerByAttributeAction(BaseAction):
    """Action to split a line layer into multiple layers based on an attribute field."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "split_line_layer_by_attribute"
        self.name = "Split Layer by Attribute"
        self.category = "Editing"
        self.description = "Split a line layer into multiple layers based on a selected attribute field. Each unique value creates a new layer containing all features with that value. All other attribute fields are preserved in the split layers."
        self.enabled = True
    ...
    def execute(self, context):
        """Execute the split line layer by attribute action."""
        try:
            ...
            # Record history for created layers so redo can restore them
            try:
                ...
                for new_layer, value_str, feature_count in created_layers:
                    try:
                        # Collect feature backups from the created layer
                        feat_backups = []
                        for f in new_layer.getFeatures():
                            try:
                                fb = self.create_feature_backup(f, new_layer, include_geometry=True, include_attributes=True)
                            except Exception:
                                fb = {'fid': f.id()}
                            feat_backups.append(fb)
                            all_feat_backups.append(fb)

                        # Capture renderer/style and symbol properties (best-effort)
                        renderer = None
                        style_str = None
                        sym_props = None

                        try:
                            if hasattr(new_layer, 'renderer'):
                                renderer = new_layer.renderer()
                        except Exception:
                            renderer = None

                        try:
                            if hasattr(new_layer, 'saveStyleToString'):
                                style_str = new_layer.saveStyleToString()
                        except Exception:
                            style_str = None
                        ...
                        labeling_info = None
                        try:
                            labeling_mgr = new_layer.labeling() if hasattr(new_layer, 'labeling') else None
                            if labeling_mgr is not None and new_layer.labelsEnabled():
                                try:
                        ...
                        for fld in new_layer.fields():
                            try:
                                fields_list.append({'name': fld.name(), 'qmeta_type': fld.type()})
                            except Exception:
                                fields_list.append({'name': fld.name()})
                        ...
                        layer_definitions.append(layer_def)
                        try:
                            layer_descriptors.append(hm.create_layer_descriptor(new_layer))
                        except Exception:
                            layer_descriptors.append(self.create_layer_descriptor(new_layer))
                    except Exception:
                        continue

                if layer_definitions:
                    # Record a composite create-layer entry for all created layers
                    try:
                        self.record_to_history(
                            description=f"Split layer '{original_layer_name}' into {len(layer_definitions)} layer(s)",
                            undo_type=HistoryManager.UNDO_TYPE_CREATE_LAYER,
                            can_undo=True,
                            undo_payload={'layer_definitions': layer_definitions},
                            layers=layer_descriptors,
                            features=all_feat_backups,
                            meta={'source_layer': layer.id(), 'source_layer_name': layer.name()},
                        )
                    except Exception:
                        pass
            except Exception:
...
split_line_layer_by_attribute_action = SplitLineLayerByAttributeAction()
