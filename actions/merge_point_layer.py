"""
Merge Point Layer Action for Right-click Utilities and Shortcuts Hub

Merges multiple point layers together, combining all features and attributes.
...
class LayerSelectionDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = QLabel(f"Select point layers to merge with '{self.initial_layer_name}':")
...
class MergePointLayerAction(BaseAction):
    """Action to merge multiple point layers together."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "merge_point_layer"
        self.name = "Merge Point Layer"
        self.category = "Editing"
        self.description = "Merge multiple point layers together, combining all features and attributes. Checks for attribute compatibility and allows merging even if attribute columns differ. Optionally deletes source layers after merge."
        self.enabled = True
    ...
    def _get_point_layers(self, exclude_layer):
        """
        Get all point layers from the project, excluding the specified layer.
        
        Args:
            exclude_layer: Layer to exclude from the list
            
        Returns:
            list: List of point layers
        """
    ...
    def execute(self, context):
        """Execute the merge point layer action."""
        try:
            ...
            # Get all other point layers
            other_layers = self._get_point_layers(layer1)
            
            if not other_layers:
                self.show_error("Error", "No other point layers found in the project to merge with")
                return
            ...
            if add_to_project:
                ...
                # Record history: creation of the merged layer for undo/redo support
                try:
                    from ..history_manager import HistoryManager

                    # Collect created features for backups
                    feat_backups = []
                    for f in merged_layer.getFeatures():
                        try:
                            fb = self.create_feature_backup(f, merged_layer, include_geometry=True, include_attributes=True)
                            feat_backups.append(fb)
                        except Exception:
                            continue

                    # Layer descriptor
                    layer_desc = self.create_layer_descriptor(merged_layer)

                    # Layer definition for redo (fields + features)
                    layer_def = {
                        'layer_name': merged_layer.name(),
                        'crs': merged_layer.crs().authid() if merged_layer.crs().isValid() else '',
                        'geometry_type': QgsWkbTypes.displayString(merged_layer.wkbType()),
                        'fields': [],
                        'features': feat_backups,
                    }
                    for fld in merged_layer.fields():
                        try:
                    ...
                    self.record_to_history(
                        description=f"Merged {len(all_layers_to_merge)} layers into '{merged_layer.name()}'",
                    ...
                    # Do not fail the action if history recording fails
                    pass
...
merge_point_layer_action = MergePointLayerAction()
