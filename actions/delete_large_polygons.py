...
class DeleteLargePolygonsAction(BaseAction):
    ...
    # Undo support: deleting features requires payload backup
    ...
    def execute(self, context):
        ...
        try:
            ...
            dialog = DeleteLargePolygonsDialog(
                self, layer, layer_crs, default_maximum_area, area_unit,
...
class DeleteLargePolygonsDialog(QDialog):
    ...
    def __init__(self, action, layer, layer_crs, default_maximum_area, area_unit, 
                 show_preview, confirm_deletion, show_progress, auto_commit):
        """
        ...
        try:
            ...
            if self.confirm_deletion:
                reply = QMessageBox.question(
                    self,
                    "Confirm Deletion",
                    f"Are you sure you want to delete {len(large_features)} polygon feature(s) larger than {maximum_area:.2f} {self.get_unit_display()}?\n\nThis action will be recorded in History and can be undone.",
            ...
            # Handle edit mode using the action's helper
            was_in_edit_mode, edit_mode_entered = self.action.handle_edit_mode(self.layer, "deleting large polygons")
            if was_in_edit_mode is None:
                QMessageBox.critical(self, "Error", "Could not start editing mode for the layer.")
                return

            # Backup features BEFORE deleting (use action helper)
            backups = []
            try:
                for feat in large_features:
                    backups.append(self.action.create_feature_backup(feat, self.layer))
            except Exception:
                backups = []
            ...
            try:
                ...
                if self.show_progress:
                    ...
                    self.progress_bar.setMaximum(len(large_features))
                    self.progress_bar.setValue(0)
                
                # Delete features
                deleted_count = 0
                for i, feat in enumerate(large_features):
                    fid = int(feat.id())
                    try:
                        if self.layer.deleteFeature(fid):
                            deleted_count += 1
                    except Exception:
                        # continue on failure
                        pass
                ...
                # Commit changes using action helper (shows errors)
                if self.auto_commit:
                    if not self.action.commit_changes(self.layer, "delete large polygons"):
                        # Exit edit mode if we entered it
                        self.action.exit_edit_mode(self.layer, edit_mode_entered)
                        return
                ...
                # Record history if any were deleted and commit succeeded
                if deleted_count > 0:
                    try:
                        # Record full backups so undo can restore features
                        layers = [self.action.create_layer_descriptor(self.layer)]
                        self.action.record_to_history(
                            description=f"Deleted {deleted_count} polygon feature(s) larger than {maximum_area:.2f} {self.get_unit_display()} from layer '{self.layer.name()}'",
                            undo_type='delete_feature',
                            can_undo=True,
                            undo_payload=None,
                            layers=layers,
                            features=backups or [],
                            meta={'threshold': maximum_area, 'unit': unit}
                        )
                    except Exception:
                        pass
                ...
                # Ensure we exit edit mode if we entered it
                try:
                    self.action.exit_edit_mode(self.layer, edit_mode_entered)
                except Exception:
                    pass
