...
class CalculateLineToNearestLineAction(BaseAction):
    ...
    def execute(self, context):
        ...
        try:
            ...
            # Record informational history entry for auditing
            try:
                from ..history_manager import get_history_manager
                hm = get_history_manager()

                # Build compact list of nearest features for metadata (JSON-serializable)
                nearest_meta = []
                for nf, dist in nearest_features:
                    try:
                        nearest_meta.append({'fid': int(nf.id()), 'distance': float(dist)})
                    except Exception:
                        # Fallback if conversion fails
                        nearest_meta.append({'fid': getattr(nf, 'id', None), 'distance': dist})

                meta_info = {
                    'from_fid': int(feature.id()),
                    'layer': hm.create_layer_descriptor(layer),
                    'nearest': nearest_meta,
                    'parameters': {
                        'nearest_features_count': int(nearest_features_count),
                        'exclude_self': bool(exclude_self)
                    }
                }

                description = f"Calculated distances from feature {feature.id()} to {len(nearest_meta)} nearest lines on layer {layer.name()}"
                hm.record_informational(self.action_id, self.name, description, meta=meta_info)
            except Exception:
                # History manager may not be available in test contexts; continue silently
                pass
