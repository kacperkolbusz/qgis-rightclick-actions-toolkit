"""
Number Polygons in Layer Action for Right-click Utilities and Shortcuts Hub

Creates a new integer field in a polygon layer and assigns sequential numbers
to all features based on a user-chosen ordering method (northernmost first,
largest area first, by centroid distance from origin, etc.).
"""
...
# The reference point is the single (x, y) used to represent a polygon when
# comparing positions between features (e.g. for north-to-south ordering).
# It does NOT affect area / perimeter / compactness calculations.

REFERENCE_POINTS = {
    'centroid':     'Centroid (geometric center)',
...
def _area(feature):
    geom = feature.geometry()
    return 0.0 if (geom is None or geom.isEmpty()) else geom.area()


def _perimeter(feature):
    geom = feature.geometry()
    return 0.0 if (geom is None or geom.isEmpty()) else geom.length()


def _compactness(feature):
    """Polsby–Popper score: 4π·area / perimeter².  Circle = 1.0."""
    a, p = _area(feature), _perimeter(feature)
...
SORT_METHODS = {
    ...
    'area_desc': {
        'label':      'Largest to Smallest (area)',
        'field_name': 'num_area_d',
        'uses_ref_pt': False,
        'sort_fn':    lambda f, ref: -_area(f),
    },
    'area_asc': {
        'label':      'Smallest to Largest (area)',
        'field_name': 'num_area_a',
        'uses_ref_pt': False,
        'sort_fn':    lambda f, ref: _area(f),
    },
    'perimeter_desc': {
        'label':      'Longest to Shortest (perimeter)',
        'field_name': 'num_per_d',
        'uses_ref_pt': False,
        'sort_fn':    lambda f, ref: -_perimeter(f),
    },
    'perimeter_asc': {
        'label':      'Shortest to Longest (perimeter)',
        'field_name': 'num_per_a',
        'uses_ref_pt': False,
        'sort_fn':    lambda f, ref: _perimeter(f),
    },
    'compactness_desc': {
        'label':      'Most to Least Compact (circle-like first)',
        'field_name': 'num_cmp_d',
        'uses_ref_pt': False,
        'sort_fn':    lambda f, ref: -_compactness(f),
    },
    'compactness_asc': {
        'label':      'Least to Most Compact (elongated first)',
        'field_name': 'num_cmp_a',
        'uses_ref_pt': False,
        'sort_fn':    lambda f, ref: _compactness(f),
    },
...
# Group the methods into labelled sections for the dialog
SORT_METHOD_GROUPS = [
    (
        "Position",
    ...
    (
        "Size",
        ['area_desc', 'area_asc', 'perimeter_desc', 'perimeter_asc'],
    ),
    (
        "Shape",
        ['compactness_desc', 'compactness_asc'],
    ),
    (
        "Other",
...
class NumberPolygonsDialog(QDialog):
    """
    ...
    def __init__(self, layer_name, ref_point_label='Centroid (geometric center)',
                 layer_fields=None, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Number Polygons")
        self.setModal(True)
        self.setMinimumWidth(430)

        # {method_key: QCheckBox}
        self._checkboxes = {}
        ...
        btn_row = QHBoxLayout()
        btn_row.addStretch()
        self._ok_btn = QPushButton("Number Polygons")
        ...
        # Wire up checkbox state → enable OK
    ...
    def selected_methods(self):
        """Return list of method descriptors (static and attribute-based) in order."""
        result = []
...
class NumberPolygonsLayerAction(BaseAction):
    """
    Action to number all polygon features in a layer.
    ...
    def __init__(self):
        ...
        self.action_id = "number_polygons_layer"
        self.name = "Number Polygons"
        self.category = "Analysis"
        self.description = (
            "Adds a sequential number field to a polygon layer. "
            "The numbering order is configurable: by position (north/south/east/west), "
            "by size (area or perimeter), by shape compactness, or by proximity to origin. "
        ...
        # runtime state used by get_undo_payload
    ...
    def get_settings_schema(self):
        return {
            'reference_point': {
                ...
                'description': (
                    'Which point on each polygon is used for position-based ordering '
                    '(North/South/East/West and Distance from Origin). '
                    'Has no effect on area, perimeter, compactness, or FID ordering. '
                    'Centroid is the geometric centre; bbox_center is the mid-point of the bounding box; '
                    'northernmost/southernmost/easternmost/westernmost are the extreme corners of the bounding box.'
                ),
    ...
    def execute(self, context):
        """Number all polygon features using one or more sort methods chosen in the dialog."""
        ...
        if not self.confirm_action("Number Polygons", msg):
            return

        # ── Enter edit mode once for all fields ────────────────────────────────
        edit_result = self.handle_edit_mode(layer, "numbering polygons")
        ...
        try:
            ...
            for spec in method_specs:
                ...
                # ── Collect old values ─────────────────────────────────────────
                # New fields have NULL old values; for existing ones re-fetch
                # fresh features so their attribute array includes the field.
                old_values = {}
            ...
            self.record_to_history(
                description=(
                    f"Numbered {len(features)} polygon(s) in '{layer.name()}' — "
                    + ", ".join(
            ...
            self.show_info(
                "Polygons Numbered",
                f"Numbered {len(features)} polygon(s) on layer '{layer.name()}'.\n\n"
                f"Fields written:\n{summary}"
            )
            ...
            try:
                ...
                pass
            self.show_error("Unexpected Error", f"An error occurred while numbering polygons: {exc}")
    ...
    def get_undo_payload(self, context, execute_result=None):
        """Return the stored undo state after a successful execute()."""
        if self._undo_state is None:
            return {}
        payload = dict(self._undo_state)
        self._undo_state = None   # consume it
        return payload


# ── Global instance for automatic discovery ───────────────────────────────────
number_polygons_layer = NumberPolygonsLayerAction()
