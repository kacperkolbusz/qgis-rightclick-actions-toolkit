"""
See Info Line Action for Right-click Utilities and Shortcuts Hub

Displays comprehensive information about the selected line feature, including
geometry details, attributes, layer information, and any available audit data.
"""

from .base_action import BaseAction
from qgis.core import QgsFeature, QgsVectorLayer, QgsWkbTypes, QgsMessageLog, Qgis
...
class SeeInfoLineAction(BaseAction):
    """
    Action to display comprehensive information about line features.
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "see_info_line"
        self.name = "See Info"
        self.category = "Information"
        self.description = "Display comprehensive information about the selected line feature, including geometry details, attributes, layer information, and any available audit data."
        self.enabled = True
    ...
    def get_settings_schema(self):
        ...
        return {
            ...
            'show_geometry_history': {
                ...
                'description': 'Display geometry information (length, vertex count, etc.)',
            },
    ...
    def execute(self, context):
        """
        Execute the see history action.
    ...
    def _get_creation_info(self, feature, layer, date_format, show_field_names):
        ...
        # (check if they're the earliest date field or have creation-like values)
        date_fields = []
        for field in fields:
    ...
    def _get_modification_info(self, feature, layer, date_format, show_field_names):
        ...
        # (check if they're the latest date field or have modification-like values)
        date_fields = []
        for field in fields:
            field_type = field.type()
            field_name = field.name()
    ...
    def _get_audit_fields(self, feature, layer, date_format, show_field_names):
        ...
        # This helps users see what fields exist that might contain history info
        if len(info_lines) < 3:
                ...
                if original_geom and current_geom:
                    if not original_geom.equals(current_geom):
                        ...
                        try:
                            ...
                            current_length = current_geom.length()
                            length_change = current_length - original_length
                            info_lines.append(f"  Length: {original_length:.2f} → {current_length:.2f} ({length_change:+.2f})")
                        except:
                            pass
    ...
    def _get_geometry_info(self, feature, layer, show_field_names):
        """Get detailed geometry information."""
        info_lines = []
        ...
        try:
            ...
            if geom_type == QgsWkbTypes.LineGeometry:
                if QgsWkbTypes.isMultiType(wkb_type):
                    info_lines.append("Geometry Type: MultiLineString")
                    # Count parts
                    try:
                        multi_line = geometry.asMultiPolyline()
                        info_lines.append(f"Number of Parts: {len(multi_line)}")
                    except:
                        pass
                else:
                    info_lines.append("Geometry Type: LineString")
            ...
            try:
                if crs.isGeographic():
                    ...
                    unit_name = "map units"
                info_lines.append(f"Length: {length:.2f} {unit_name}")
            except:
                info_lines.append(f"Length: {length:.2f} map units")
            ...
            try:
                ...
                # Show first and last vertex coordinates
                if vertex_count > 0:
                    first_vertex = vertices[0]
                    last_vertex = vertices[-1]
                    info_lines.append(f"Start Point: ({first_vertex.x():.2f}, {first_vertex.y():.2f})")
                    info_lines.append(f"End Point: ({last_vertex.x():.2f}, {last_vertex.y():.2f})")
            except:
                pass
            ...
            # Area (for closed lines/polygons)
            try:
                area = geometry.area()
                if area > 0:
                    crs = layer.crs()
...
see_info_line = SeeInfoLineAction()
