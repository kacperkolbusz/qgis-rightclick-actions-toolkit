    ...
    def execute(self, context):
        ...
        try:
            ...
            if create_copy:
                ...
                # Add using the provider where possible so new FID is assigned
                added_ok = False
                added_feature_obj = None
                try:
                    provider = layer.dataProvider()
                    if provider:
                        res = provider.addFeatures([new_feature])
                        # provider.addFeatures may return (success, added_feats) or boolean
                        if isinstance(res, tuple) and len(res) >= 2:
                            success, added_feats = res[0], res[1]
                            if success and added_feats:
                                added_ok = True
                                added_feature_obj = added_feats[0]
                        elif isinstance(res, bool):
                            added_ok = res
                except Exception:
                    added_ok = False

                # Fallback to layer.addFeature if provider.addFeatures not available
                if not added_ok:
                    try:
                        added_ok = layer.addFeature(new_feature)
                        # when layer.addFeature succeeds, try to get the newly added feature object
                        if added_ok:
                            # attempt to find feature by geometry
                            try:
                                geom_wkb = new_feature.geometry().asWkb()
                                for f in layer.getFeatures():
                                    try:
                                        if f.geometry() and f.geometry().asWkb() == geom_wkb:
                                            added_feature_obj = f
                                            break
                                    except Exception:
                                        continue
                ...
                # Ensure layer sees new extents
                try:
                    layer.updateExtents()
                except Exception:
                    pass

                # Use the added feature object if available (it has correct FID)
                feature_to_update = added_feature_obj if added_feature_obj is not None else new_feature
                operation_type = "copy"
            else:
                # Backup original feature before modifying
                try:
            ...
            # Record history after successful commit
            try:
                layers = [self.create_layer_descriptor(layer)]
                if operation_type == "copy":
                    # Create backup for the created feature
                    try:
                        created_backup = self.create_feature_backup(feature_to_update, layer)
                    except Exception:
                        created_backup = {'fid': feature_to_update.id()}

                    self.record_to_history(
                        description=f"Created generalized copy of feature {feature.id()} on layer '{layer.name()}'",
                        undo_type='create_feature',
                        can_undo=True,
                        undo_payload=None,
                        layers=layers,
                        features=[created_backup],
                        meta={'tolerance': tolerance}
                    )
                else:
                    # Update geometry undo entry - include old_geometry and new_geometry
                    try:
                        old_geom = None
                        if 'original_backup' in locals() and original_backup:
                            old_geom = original_backup.get('geometry')
                        else:
                            # fallback: create backup from original feature (if possible)
                            try:
                                old_geom = self.create_feature_backup(feature_to_update, layer).get('geometry')
                            except Exception:
                                old_geom = None
                        ...
                        new_geom_wkb = None
                        try:
                            new_geom_wkb = generalized_geometry.asWkb()
                        except Exception:
                            try:
                                new_geom_wkb = feature_to_update.geometry().asWkb()
                            except Exception:
                                new_geom_wkb = None
                        ...
                        feat_payload = {'fid': feature_to_update.id()}
                        if old_geom:
                            feat_payload['old_geometry'] = old_geom
                        if new_geom:
                            feat_payload['new_geometry'] = new_geom

                    except Exception:
                        feat_payload = {'fid': feature_to_update.id()}

                    self.record_to_history(
                        description=f"Generalized line feature {feature.id()} on layer '{layer.name()}'",
