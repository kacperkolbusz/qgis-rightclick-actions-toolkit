    ...
    def execute(self, context):
        ...
        try:
            ...
            if add_to_project:
                ...
                # Record history for created layers (store style + labeling to allow exact redo)
                try:
                    ...
                    # Helper to build layer definition
                    def build_layer_def(layer_obj):
                        try:
                            # Collect feature backups
                            created_feats = list(layer_obj.getFeatures())
                            feat_backups = []
                            for f in created_feats:
                                try:
                                    fb = hm.create_feature_backup(f, layer_obj, include_geometry=True, include_attributes=True)
                            ...
                            try:
                                ...
                                style_str = None
                                try:
                                    if hasattr(layer_obj, 'saveStyleToString'):
                                        try:
                                            style_str = layer_obj.saveStyleToString()
                                        except Exception:
                                            style_str = None
                                except Exception:
                                    style_str = None
                            ...
                            # Capture labeling info
                            labeling_info = None
                            try:
                                labeling_mgr = layer_obj.labeling() if hasattr(layer_obj, 'labeling') else None
                                if labeling_mgr is not None and layer_obj.labelsEnabled():
                                    try:
                            ...
                            return {
                                ...
                                'crs': layer_obj.crs().authid() if layer_obj.crs().isValid() else '',
                                'geometry_type': QgsWkbTypes.displayString(layer_obj.wkbType()),
                                'fields': fields_list,
                    ...
                    out_def = build_layer_def(output_layer)
                    if out_def:
                        layer_definitions.append(out_def)

                    if arc_layer:
                        arc_def = build_layer_def(arc_layer)
                        if arc_def:
                            layer_definitions.append(arc_def)

                    if layer_definitions:
                        # Record a composite create-layer entry for both layers (if present)
                        hm.record(
                            action_id=self.action_id,
                            action_name=self.name,
                            description=f"Created angle layer(s) '{output_layer.name()}'{(' and ' + arc_layer.name()) if arc_layer else ''}",
                            undo_type=HistoryManager.UNDO_TYPE_CREATE_LAYER,
                            can_undo=True,
                            undo_payload={'layer_definitions': layer_definitions},
                            layers=[hm.create_layer_descriptor(output_layer)] + ([hm.create_layer_descriptor(arc_layer)] if arc_layer else []),
                            features=None,
                            atomic=True,
