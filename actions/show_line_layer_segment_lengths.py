    ...
    def execute(self, context):
        ...
        try:
            ...
            if add_to_project:
                ...
                # Record history: creation of the new segment lengths layer
                try:
                    ...
                    created_features = list(segment_layer.getFeatures())
                    feat_backups = []
                    for f in created_features:
                        fb = hm.create_feature_backup(f, segment_layer, include_geometry=True, include_attributes=True)
                        feat_backups.append(fb)

                    # Layer descriptor for quick refs
                    layer_desc = hm.create_layer_descriptor(segment_layer)

                    # Try to capture renderer information and simple labeling settings
                    renderer_info = None
                    try:
                        renderer = segment_layer.renderer() if hasattr(segment_layer, 'renderer') else None
                        sym_props = None
                        try:
                        ...
                        # Try to serialize full style if available
                        style_str = None
                        try:
                            if hasattr(segment_layer, 'saveStyleToString'):
                                try:
                                    style_str = segment_layer.saveStyleToString()
                                except Exception:
                                    style_str = None
                        except Exception:
                            style_str = None
                    ...
                    labeling_info = None
                    try:
                        labeling_mgr = segment_layer.labeling() if hasattr(segment_layer, 'labeling') else None
                        if labeling_mgr is not None and segment_layer.labelsEnabled():
                            try:
                            ...
                            if settings is not None:
                                ...
                                # Try to extract text size if available
                                size = None
                                try:
                    ...
                    layer_def = {
                        'layer_name': segment_layer.name(),
                        'crs': segment_layer.crs().authid() if segment_layer.crs().isValid() else '',
                        'geometry_type': QgsWkbTypes.displayString(segment_layer.wkbType()),
                        'fields': [],
                        'features': feat_backups,
                        'labeling': labeling_info,
                        'renderer': renderer_info,
                    }
                    for fld in segment_layer.fields():
                        try:
                    ...
                    hm.record(
                        ...
                        description=f"Created segment lengths layer '{segment_layer.name()}' with {len(feat_backups)} feature(s)",
