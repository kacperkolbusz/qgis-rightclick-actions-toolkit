...
class MeasureDistanceAction(BaseAction):
    ...
    # Undo system: this action is purely informational/interactive and does not
    # modify project data, so it must not be recorded as an undoable operation.
    def supports_undo(self) -> bool:
        """
        Explicitly declare that this action does not support undo.
        """
        return False

    def get_undo_category(self) -> str:
        """
        Return 'informational' to indicate this is a display-only action
        and should not be treated as undoable by the History Manager.
        """
    ...
    def execute(self, context):
        ...
        try:
            ...
            # Record informational entry in history (non-undoable)
            try:
                desc = f"Started distance measurement at ({first_point.x():.{decimal_places}f}, {first_point.y():.{decimal_places}f})"
                self.record_informational(description=desc, meta={
                    'first_point': {'x': float(first_point.x()), 'y': float(first_point.y())},
                    'crs': canvas_crs.authid() if canvas_crs.isValid() else ''
                })
            except Exception:
                # History recording should not interrupt the measurement flow
                pass
