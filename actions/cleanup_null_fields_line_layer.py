"""
Cleanup NULL Fields - Line Layer Action for Right-click Utilities and Shortcuts Hub

Analyzes the attribute table of a line layer and removes fields where a configurable
...
class CleanupNullFieldsLineLayerAction(BaseAction):
    """Action to remove high-NULL fields from line layer attribute tables."""

    def __init__(self):
        """Initialize the action with metadata and configuration."""
        super().__init__()

        self.action_id = 'cleanup_null_fields_line_layer'
        self.name = 'Clean Up NULL Fields'
        self.category = 'Editing'
        self.description = (
            'Analyzes the attribute table of a line layer and deletes fields where '
    ...
    def execute(self, context):
        """Execute the NULL field cleanup action on a line layer."""
        ...
        self.record_to_history(
            description=(
                ...
                f"line layer '{layer.name()}' (threshold: {null_threshold:.0f}%)"
            ),
...
cleanup_null_fields_line_layer = CleanupNullFieldsLineLayerAction()
