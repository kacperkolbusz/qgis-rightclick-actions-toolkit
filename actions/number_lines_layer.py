"""
Number Lines in Layer Action for Right-click Utilities and Shortcuts Hub

Creates a new integer field in a line layer and assigns sequential numbers
to all features based on a user-chosen ordering method (northernmost first,
longest first, most sinuous first, by centroid distance from origin, etc.).
"""
...
# The reference point is the single (x, y) used to represent a line when
# comparing positions between features (e.g. for north-to-south ordering).
# It does NOT affect length or sinuosity calculations.

REFERENCE_POINTS = {
    'centroid':     'Centroid (geometric center)',
    'bbox_center':  'Bounding box center',
    'start_point':  'Start point (first vertex)',
    'end_point':    'End point (last vertex)',
    'midpoint':     'Midpoint (50 % along line)',
...
def _line_length(feature):
    """Return the total length of a line feature's geometry."""
    geom = feature.geometry()
    return 0.0 if (geom is None or geom.isEmpty()) else geom.length()


def _sinuosity(feature):
    """
    Return the sinuosity of a line: total length / straight-line distance
    between first and last vertex.  A perfectly straight line scores 1.0;
    higher values indicate more winding.  Closed lines return infinity.
    """
    geom = feature.geometry()
    if geom is None or geom.isEmpty():
        return 1.0
    ...
    try:
        ...
        if straight == 0:
            return float('inf')   # Closed line – maximally sinuous
...
SORT_METHODS = {
    ...
    'length_desc': {
        'label':       'Longest to Shortest (length)',
        'field_name':  'num_len_d',
        'uses_ref_pt': False,
        'sort_fn':     lambda f, ref: -_line_length(f),
    },
    'length_asc': {
        'label':       'Shortest to Longest (length)',
        'field_name':  'num_len_a',
        'uses_ref_pt': False,
        'sort_fn':     lambda f, ref: _line_length(f),
    },
    'sinuosity_desc': {
        'label':       'Most to Least Sinuous (winding first)',
        'field_name':  'num_sin_d',
        'uses_ref_pt': False,
        'sort_fn':     lambda f, ref: -_sinuosity(f),
    },
    'sinuosity_asc': {
        'label':       'Least to Most Sinuous (straight first)',
        'field_name':  'num_sin_a',
        'uses_ref_pt': False,
        'sort_fn':     lambda f, ref: _sinuosity(f),
    },
...
class NumberLinesDialog(QDialog):
    """
    ...
    def __init__(self, layer_name, ref_point_label='Centroid (geometric center)',
        ...
        self.setWindowTitle("Number Lines")
        self.setModal(True)
        ...
        btn_row = QHBoxLayout()
        btn_row.addStretch()
        self._ok_btn = QPushButton("Number Lines")
...
class NumberLinesLayerAction(BaseAction):
    """
    Action to number all line features in a layer.
    ...
    def __init__(self):
        ...
        self.action_id = "number_lines_layer"
        self.name = "Number Lines"
        self.category = "Analysis"
        self.description = (
            "Adds a sequential number field to a line layer. "
            "The numbering order is configurable: by position (north/south/east/west), "
            "by length (longest/shortest), by sinuosity (winding vs straight), "
    ...
    def get_settings_schema(self):
        return {
            'reference_point': {
                ...
                'description': (
                    'Which point on each line is used for position-based ordering '
                    '(North/South/East/West and Distance from Origin). '
                    'Has no effect on length, sinuosity, or FID ordering. '
                    'Centroid is the geometric centre of the line; start/end point are the first/last vertices; '
                    'midpoint is the coordinate 50 % along the line; '
    ...
    def execute(self, context):
        """Number all line features using one or more sort methods chosen in the dialog."""
        ...
        if not self.confirm_action("Number Lines", msg):
            return

        # ── Enter edit mode once for all fields ────────────────────────────────
        edit_result = self.handle_edit_mode(layer, "numbering lines")
        ...
        try:
            ...
            self.record_to_history(
                description=(
                    f"Numbered {len(features)} line(s) in '{layer.name()}' — "
                    + ", ".join(
            ...
            self.show_info(
                "Lines Numbered",
                f"Numbered {len(features)} line(s) on layer '{layer.name()}'.\n\n"
                f"Fields written:\n{summary}"
            )
            ...
            try:
                ...
                pass
            self.show_error("Unexpected Error", f"An error occurred while numbering lines: {exc}")
...
number_lines_layer = NumberLinesLayerAction()
