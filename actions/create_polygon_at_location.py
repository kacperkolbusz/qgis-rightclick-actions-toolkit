...
class CreatePolygonAtLocationAction(BaseAction):
    ...
    def _create_polygon_from_vertices(self, vertices, crs, settings):
        ...
        try:
            ...
            if existing_layer:
                ...
                created_fid = self._add_polygon_to_layer(existing_layer, polygon_geometry, crs, settings)
                ...
                if created_fid is not None:
                    ...
                    # Record to history for undo (CREATE_FEATURE)
                    try:
                        hm = get_history_manager()
                        hm.record(
                            action_id=self.action_id,
                            action_name=self.name,
                            description=f"Created polygon in existing layer '{settings['layer_name']}'",
                ...
                polygon_layer, created_fid = self._create_polygon_layer(
                ...
                if created_fid is not None:
                    ...
                    from qgis.core import QgsFeatureRequest
                    created_feature = None
                    for f in polygon_layer.getFeatures(QgsFeatureRequest().setFilterFid(created_fid)):
                        created_feature = f
                        break

                    feature_backup = self.create_feature_backup(created_feature, polygon_layer) if created_feature else {'fid': created_fid}

                    # Layer definition to allow redo of the layer (structure + backups)
                    layer_def = {
                        'layer_name': polygon_layer.name(),
                        'crs': polygon_layer.crs().authid() if polygon_layer.crs().isValid() else '',
                        'geometry_type': QgsWkbTypes.displayString(polygon_layer.wkbType()),
                        'fields': [],
                        'features': [feature_backup],
                    }
                    for fld in polygon_layer.fields():
                        try:
                    ...
                    try:
                        ...
                        hm.record(
                            action_id=self.action_id,
                            action_name=self.name,
                            description=f"Created polygon in new layer '{polygon_layer.name()}'",
                            undo_type=HistoryManager.UNDO_TYPE_CREATE_LAYER,
                            can_undo=True,
                            undo_payload={'layer_definition': layer_def},
                            layers=[self.create_layer_descriptor(polygon_layer)],
                            features=[feature_backup],
                            atomic=True,
    ...
    def _create_polygon_layer(self, layer_name, geometry, crs, settings):
        ...
        try:
            ...
            # Add feature to layer using dataProvider to obtain FID
