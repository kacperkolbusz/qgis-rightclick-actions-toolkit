"""
Rotate Line Layer Action for Right-click Utilities and Shortcuts Hub

Rotates all line features in a layer by a specified angle (degrees).
...
class RotateLineLayerDialog(QDialog):
    """Dialog for layer-level rotation configuration."""
    ...
    def __init__(self, parent=None, default_angle=0.0, feature_count=None, selected_count=None, ask_copy=True, default_copy=False):
        ...
        self.setWindowTitle("Rotate Line Layer")
        self.setModal(True)
        self.resize(380, 200)
...
class RotateLineLayerAction(BaseAction):
    """Layer-level rotation for line layers with undo support."""

    def __init__(self):
        super().__init__()
        self.action_id = "rotate_line_layer"
        self.name = "Rotate Line Layer"
        self.category = "Editing"
        self.description = "Rotate all line features in a layer by a specified angle. Supports selected-only processing and creating a rotated copy layer. Records undo payloads for geometry updates."
        self.enabled = True
    ...
    def execute(self, context):
        ...
        dialog = RotateLineLayerDialog(None, default_angle=default_angle, feature_count=feature_count, selected_count=(selected_count if selected_count>0 else None), ask_copy=show_copy_option, default_copy=default_copy)
        ...
        if create_copy:
            ...
            new_layer = QgsVectorLayer(f"MultiLineString?crs={layer_crs}", f"{layer.name()} (Rotated)", "memory")
        ...
        # PATH B: Modify original layer in-place
            ...
            if handle_edit_mode:
                edit_result = self.handle_edit_mode(layer, "line layer rotation")
            ...
            try:
                ...
                if auto_commit and handle_edit_mode:
                    if not self.commit_changes(layer, "line layer rotation"):
                        return
...
rotate_line_layer = RotateLineLayerAction()
