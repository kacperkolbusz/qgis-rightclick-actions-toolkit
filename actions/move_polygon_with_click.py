"""
Move Polygon Action for Right-click Utilities and Shortcuts Hub

Moves the selected polygon feature to a new location specified by the user.
User clicks on the map to specify the new location for the polygon.
"""
...
class MovePolygonMapTool(QgsMapTool):
    """Custom map tool for moving polygon features."""
    ...
    def canvasPressEvent(self, event):
        """Handle canvas press to place polygon at new location."""
    ...
    def _cancel_move(self):
        ...
        if self.settings.get('show_cancellation_message', True):
            self.parent_action.show_info("Move Cancelled", "Polygon move operation was cancelled.")
...
class MovePolygonAction(BaseAction):
    """Action to move polygon features to a new location."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "move_polygon_with_click"
        self.name = "Move Polygon with Click"
        self.category = "Editing"
        self.description = "Move the selected polygon feature to a new location. Click on the map to specify where to move the polygon. Right-click or press Escape to cancel."
        self.enabled = True
    ...
    def get_settings_schema(self):
        ...
        return {
            ...
            'confirmation_message_template': {
                'type': 'str',
                'default': 'Move polygon feature ID {feature_id} from layer \'{layer_name}\'?',
            ...
            'success_message_template': {
                'type': 'str',
                'default': 'Polygon feature ID {feature_id} moved successfully',
    ...
    def execute(self, context):
        """
        Execute the move polygon action.
        ...
        try:
            ...
            confirmation_template = str(self.get_setting('confirmation_message_template', 'Move polygon feature ID {feature_id} from layer \'{layer_name}\'?'))
            show_success = bool(self.get_setting('show_success_message', True))
            success_template = str(self.get_setting('success_message_template', 'Polygon feature ID {feature_id} moved successfully'))
            ...
            show_polygon_area = bool(self.get_setting('show_polygon_area_info', False))
        ...
        if use_unified_dialog and (confirm_move or show_copy_option):
            ...
            if confirm_move:
                if not self.confirm_action("Move Polygon", confirmation_message):
                    return
        ...
        move_tool = MovePolygonMapTool(canvas, self, feature, layer, geometry)
    ...
    def _move_feature_to_geometry(self, feature, layer, new_geometry):
        ...
        try:
            ...
            # Preserve original geometry for undo backups
            original_geom = None
            try:
                original_geom = original_geometry
            except Exception:
                original_geom = None
            ...
            if create_copy:
                ...
                new_feature.setId(-1)  # Let provider assign real FID
                ...
                # Use added feature (has real FID)
                new_feature = added[0]
            ...
            # Prepare history recording data
            ...
            try:
                if original_geom is not None:
                    old_geom_wkb = original_geom.asWkb()
            ...
            try:
                if create_copy:
                    ...
                    self.record_to_history(
                        description=f"Moved feature {feature.id()} in layer {layer.name()} to new location",
            ...
            if settings.get('show_success_message', True):
                if create_copy and new_feature:
                    ...
                    success_message = self.format_message_template(
                        settings.get('success_message_template', 'Polygon feature ID {feature_id} moved successfully'),
...
move_polygon_with_click_action = MovePolygonAction()
