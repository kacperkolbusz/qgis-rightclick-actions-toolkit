"""
See Info Point Action for Right-click Utilities and Shortcuts Hub

Displays comprehensive information about the selected point feature, including
geometry details, attributes, layer information, and any available audit data.
"""

from .base_action import BaseAction
from qgis.core import QgsFeature, QgsVectorLayer, QgsWkbTypes, QgsPoint, QgsMessageLog, Qgis
...
class SeeInfoPointAction(BaseAction):
    """
    Action to display comprehensive information about point features.
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "see_info_point"
        self.name = "See Info"
        self.category = "Information"
        self.description = "Display comprehensive information about the selected point feature, including geometry details, attributes, layer information, and any available audit data."
        self.enabled = True
    ...
    def get_settings_schema(self):
        ...
        return {
            ...
            'show_geometry_history': {
                ...
                'description': 'Display geometry information (coordinates, Z/M values, etc.)',
            },
            ...
            'display_mode': {
                ...
                'default': 'dialog',
                'label': 'Display Mode',
                'description': '"dialog" opens a floating window. "panel" docks the info into the QGIS interface and updates it each time you click (great for browsing many features). "message" shows a simple info box.',
                'options': ['dialog', 'panel', 'message'],
            },
            'copy_to_clipboard': {
                'type': 'bool',
                'default': False,
    ...
    def execute(self, context):
        ...
        try:
            ...
            display_mode = str(self.get_setting('display_mode', 'dialog'))
        ...
        try:
            ...
            # Display info
            feature_title = f"Feature Info — {layer.name()} [ID: {feature.id()}]"
            if display_mode == 'panel':
                self._show_in_panel(info_text, feature_title)
            elif display_mode == 'dialog':
            ...
            try:
                self.record_informational(
                    description=f"Viewed feature information for feature {feature.id()} in layer {layer.name()}",
                    meta={
                ...
                try:
                    QgsMessageLog.logMessage(
                        f"Recorded informational history for {self.action_id}: feature {feature.id()} layer {layer.name()}",
                        'RightClickActionsToolkit',
                        Qgis.Info
                    )
                except Exception:
                    pass
    ...
    def _show_in_panel(self, info_text, title):
        """Display feature information in a docked QGIS panel.

        The panel is reused on subsequent calls so clicking different features
        updates the content in place rather than opening new windows.
        """
        try:
            from qgis.utils import iface
            from qgis.gui import QgsDockWidget
            from qgis.PyQt.QtWidgets import QWidget, QVBoxLayout, QTextEdit, QPushButton, QApplication
            from qgis.PyQt.QtCore import Qt

            panel_name = f"RATFeatureInfoPanel_{self.action_id}"
            main_window = iface.mainWindow()

            # Reuse existing panel if it is already open
            existing = main_window.findChild(QgsDockWidget, panel_name)
            if existing:
                text_edit = existing.widget().findChild(QTextEdit)
                if text_edit:
            ...
            # Create a new docked panel
            dock = QgsDockWidget(title, main_window)
                ...
                if original_geom and current_geom:
                    if not original_geom.equals(current_geom):
                        ...
                        try:
                            ...
                            current_point = current_geom.asPoint()
                            info_lines.append(f"  Position: ({original_point.x():.2f}, {original_point.y():.2f}) → ({current_point.x():.2f}, {current_point.y():.2f})")
                        except:
                            pass
    ...
    def _get_geometry_info(self, feature, layer, show_field_names):
        """Get detailed geometry information for point."""
        info_lines = []
        ...
        try:
            ...
            if geom_type == QgsWkbTypes.PointGeometry:
                if QgsWkbTypes.isMultiType(wkb_type):
                    info_lines.append("Geometry Type: MultiPoint")
                    # Count parts
                    try:
                        multi_point = geometry.asMultiPoint()
                        info_lines.append(f"Number of Points: {len(multi_point)}")
                        # Show coordinates for each point
                        for i, point in enumerate(multi_point):
                            info_lines.append(f"  Point {i+1}: ({point.x():.2f}, {point.y():.2f})")
                    except:
                        pass
                else:
                    info_lines.append("Geometry Type: Point")
            
            # Point-specific info - Coordinates (primary metric)
            try:
                point = geometry.asPoint()
                info_lines.append(f"X Coordinate: {point.x():.2f}")
                info_lines.append(f"Y Coordinate: {point.y():.2f}")
                
                # Check for Z coordinate
                if wkb_type in [QgsWkbTypes.PointZ, QgsWkbTypes.MultiPointZ]:
                    try:
                        point_3d = geometry.asPoint()
                        if hasattr(point_3d, 'z') and point_3d.z() is not None:
                            info_lines.append(f"Z Coordinate: {point_3d.z():.2f}")
                    except:
                        pass
                
                # Check for M coordinate
                if wkb_type in [QgsWkbTypes.PointM, QgsWkbTypes.MultiPointM, QgsWkbTypes.PointZM, QgsWkbTypes.MultiPointZM]:
                    try:
                        point_m = geometry.asPoint()
                        if hasattr(point_m, 'm') and point_m.m() is not None:
                            info_lines.append(f"M Coordinate: {point_m.m():.2f}")
                    except:
                        pass
            except:
                pass
            
            # Bounding box (for point, this is just the point itself)
...
see_info_point = SeeInfoPointAction()
