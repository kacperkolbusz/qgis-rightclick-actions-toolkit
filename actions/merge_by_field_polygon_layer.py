"""
Merge Polygons By Field (Layer Action)

Group polygon features by a chosen field value and union their geometries into a
single geometry per distinct value. Original features are removed and merged
...
class MergePolygonsByFieldLayerAction(BaseAction):
    """Merge polygon features by a chosen attribute field on the entire layer."""

    def __init__(self):
        super().__init__()
        self.action_id = "merge_polygons_by_field_layer"
        self.name = "Merge Polygons By Field"
        self.category = "Editing"
        self.description = "Merge polygon features in the layer by an attribute field. Features with the same value are combined into a single geometry per value."
        self.enabled = True
...
merge_polygons_by_field_layer_action = MergePolygonsByFieldLayerAction()
