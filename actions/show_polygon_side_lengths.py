"""
Show Polygon Side Lengths Action for Right-click Utilities and Shortcuts Hub

Displays the length of each side in a polygon feature by creating labeled points
at the midpoint of each side showing the side length.
"""
...
class ShowPolygonSideLengthsAction(BaseAction):
    """Action to display side lengths on polygon features."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "show_polygon_side_lengths"
        self.name = "Show Polygon Side Lengths"
        self.category = "Information"
        self.description = "Display the length of each side in a polygon feature. Creates labeled points at the midpoint of each side showing the side length in map units. Works with polygon and multipolygon features."
        self.enabled = True
    ...
    def get_settings_schema(self):
        ...
        return {
            ...
            'layer_name_template': {
                'type': 'str',
                'default': 'Side Lengths_{feature_id}',
                'label': 'Layer Name Template',
                'description': 'Template for the side lengths layer name. Available variables: {feature_id}, {layer_name}, {timestamp}',
            },
            'add_to_project': {
                'type': 'bool',
                'default': True,
            ...
            'include_perimeter': {
                ...
                'description': 'Show total perimeter in information message',
            },
    ...
    def _get_polygon_sides(self, geometry, crs):
        ...
        if not geometry or geometry.isEmpty():
            print("Warning: Geometry is empty or None")
            return sides
        ...
        try:
            if not geometry.isGeosValid():
                ...
                if geometry.isEmpty():
                    print("Warning: Geometry became empty after makeValid()")
                    return sides
        except Exception as e:
        ...
        for i in range(len(points)):
            ...
            # Calculate length using the SAME method as show_line_segment_lengths
    ...
    def _create_side_lengths_layer(self, layer_name, crs, include_side_index):
        """
    ...
    def _enable_labeling(self, layer, length_field_name, decimal_places, label_size, label_color, label_placement, include_side_index):
        """
        ...
        try:
            ...
            if include_side_index:
                ...
                expression = f'to_string("side_index") || ": " || format_number("{length_field_name}", {decimal_places})'
            else:
    ...
    def execute(self, context):
        """Execute the show polygon side lengths action."""
        ...
        try:
            ...
            include_perimeter = bool(self.get_setting('include_perimeter', schema['include_perimeter']['default']))
        ...
        try:
            ...
            try:
                ...
                print(f"Exception in _get_polygon_sides: {error_details}")
                
                error_msg = (
                    f"Failed to extract polygon sides.\n\n"
                    f"Error: {str(e)}\n\n"
            ...
            if not sides:
                ...
                error_msg = (
                    f"Could not extract sides from polygon geometry.\n\n"
            ...
            for side_info in side_data:
                ...
                if include_side_index:
                    attributes.append(side_info['index'])
            ...
            if add_to_project:
                ...
                # Record history: creation of the side lengths layer so redo can restore styling and labels
                try:
                    from ..history_manager import get_history_manager, HistoryManager

                    hm = get_history_manager()

                    # Collect created features backups
                    created_features = list(side_layer.getFeatures())
                    feat_backups = []
                    for f in created_features:
                        fb = hm.create_feature_backup(f, side_layer, include_geometry=True, include_attributes=True)
                        feat_backups.append(fb)

                    # Capture renderer info and try to save full style where possible
                    renderer_info = None
                    try:
                        renderer = side_layer.renderer() if hasattr(side_layer, 'renderer') else None
                        sym_props = None
                        try:
                            if renderer is not None:
                                try:
                                    sls = renderer.symbol().symbolLayers()
                        ...
                        style_str = None
                        try:
                            if hasattr(side_layer, 'saveStyleToString'):
                                try:
                                    style_str = side_layer.saveStyleToString()
                                except Exception:
                                    style_str = None
                        except Exception:
                            style_str = None
                        ...
                        renderer_info = {
                            'renderer_type': getattr(renderer, 'type', None) if renderer is not None else None,
                    ...
                    # Capture simple labeling settings
                    labeling_info = None
                    try:
                        labeling_mgr = side_layer.labeling() if hasattr(side_layer, 'labeling') else None
                        if labeling_mgr is not None and side_layer.labelsEnabled():
                            try:
                                settings = labeling_mgr.settings()
                            except Exception:
                                settings = None

                            if settings is not None:
                                try:
                                    field_name = getattr(settings, 'fieldName', None)
                                    is_expr = getattr(settings, 'isExpression', False)
                                ...
                                size = None
                                try:
                                    fmt = settings.format() if callable(getattr(settings, 'format', None)) else getattr(settings, 'format', None)
                                    if fmt is not None:
                                        size = getattr(fmt, 'size', None) if not callable(getattr(fmt, 'size', None)) else None
                                except Exception:
                                    size = None
                    ...
                    # Layer descriptor for quick refs
                    layer_desc = hm.create_layer_descriptor(side_layer)

                    # Layer definition to allow redo of the layer (structure + backups)
                    layer_def = {
                        'layer_name': side_layer.name(),
                        'crs': side_layer.crs().authid() if side_layer.crs().isValid() else '',
                        'geometry_type': QgsWkbTypes.displayString(side_layer.wkbType()),
                        'fields': [],
                        'features': feat_backups,
                        'labeling': labeling_info,
                        'renderer': renderer_info,
                    }
                    for fld in side_layer.fields():
                        try:
                            layer_def['fields'].append({'name': fld.name(), 'qmeta_type': fld.type()})
                        except Exception:
                            layer_def['fields'].append({'name': fld.name()})

                    hm.record(
                        action_id=self.action_id,
                        action_name=self.name,
                        description=f"Created side lengths layer '{side_layer.name()}' with {len(feat_backups)} feature(s)",
                        ...
                        features=feat_backups,
                        atomic=True,
                        meta={'source_layer': layer.id(), 'source_layer_name': layer.name(), 'source_feature_id': feature_id}
                    )
                except Exception:
                    # History recording must not prevent successful action
                    pass
...
show_polygon_side_lengths = ShowPolygonSideLengthsAction()
