"""
Smooth Line Action for Right-click Utilities and Shortcuts Hub

Smooths the selected line feature using configurable smoothing algorithms.
...
class SmoothLineDialog(QDialog):
    """Unified dialog for smoothing line with copy option."""
    
    def __init__(self, parent=None, default_iterations=1, default_offset=0.25, 
                 line_length=None, ask_copy=True, default_copy=False):
        super().__init__(parent)
        self.setWindowTitle("Smooth Line")
        self.setModal(True)
        self.resize(400, 300)
        ...
        iterations_help = QLabel("More iterations = smoother result (1-10 recommended)")
...
class SmoothLineAction(BaseAction):
    """
    Action to smooth line features using configurable smoothing algorithms.
    
    This action smooths the selected line feature using Chaikin's corner cutting algorithm
    by default. The architecture allows for easy addition of other smoothing methods in the future.
    Supports creating a smoothed copy while keeping the original unchanged.
    """
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "smooth_line"
        self.name = "Smooth Line"
        self.category = "Editing"
        self.description = "Smooth the selected line feature using configurable smoothing algorithms. Uses Chaikin's corner cutting algorithm by default. Supports creating a smoothed copy while keeping the original unchanged. Configurable iterations and offset parameters control smoothing strength."
        self.enabled = True
        ...
        # Undo-related state (populated during execute)
        self._feature_backup = None          # backup of original feature (for update/delete)
        self._created_feature_backup = None  # backup of created feature (for create)
        self._target_layer = None
        self._last_operation = None          # 'copy' or 'smooth'
    ...
    def get_settings_schema(self):
        ...
        return {
            ...
            'confirm_before_smooth': {
                ...
                'description': 'Show confirmation dialog before smoothing the line',
            },
            'show_success_message': {
                'type': 'bool',
                'default': True,
                'label': 'Show Success Message',
                'description': 'Display a message when line is smoothed successfully',
            },
    ...
    def supports_undo(self) -> bool:
        """This action supports undo (modifies feature geometry)."""
        return True

    def get_undo_category(self) -> str:
        """Return undo category: geometry modifications require payload backups."""
        return 'payload'

    def get_undo_payload(self, context: dict, execute_result=None) -> dict:
        """Return undo payload after successful execution.

        The History Manager may call this to obtain a payload that is
        ...
        # For create (copy) operations we return the created feature backup
        if self._last_operation == 'copy' and self._created_feature_backup:
            return {
                'undo_type': 'create_feature',
                'layers': layers,
                'features': [self._created_feature_backup],
                'description': f"Created smoothed copy in layer {self._target_layer.name()}"
            }

        # For smoothing the original feature, return the original backup
        if self._last_operation == 'smooth' and self._feature_backup:
            return {
                'undo_type': 'update_geometry',
                'layers': layers,
                'features': [self._feature_backup],
                'description': f"Smoothed feature {self._feature_backup.get('fid')} in {self._target_layer.name()}"
            }
    ...
    def execute(self, context):
        """
        Execute the smooth line action.
        ...
        if confirm_before_smooth:
            confirmation_message = f"Smooth line feature ID {feature.id()} from layer '{layer.name()}'?\n\n"
            ...
            if not self.confirm_action("Smooth Line", confirmation_message):
                return
        ...
        if handle_edit_mode:
            edit_result = self.handle_edit_mode(layer, "line smoothing")
        ...
        try:
            ...
            if create_copy:
                ...
                # Use dataProvider().addFeatures() to get correct FID (see guide)
                try:
                    success, added_features = layer.dataProvider().addFeatures([new_feature])
                except Exception:
                    success = False
                    added_features = None
                ...
                # Get created feature id and full feature for backup/redo
                created_fid = added_features[0].id()
                from qgis.core import QgsFeatureRequest
                created_feature = None
                for f in layer.getFeatures(QgsFeatureRequest().setFilterFid(created_fid)):
                    created_feature = f
                    break

                self._created_feature_backup = self.create_feature_backup(created_feature, layer) if created_feature else {'fid': created_fid}
                feature_to_update = created_feature or added_features[0]
                self._last_operation = 'copy'
                operation_type = 'copy'

            else:
                # Backup original BEFORE modifying so undo can restore original geometry
                self._feature_backup = self.create_feature_backup(feature, layer)
                self._last_operation = 'smooth'
            ...
            if auto_commit and handle_edit_mode:
                if not self.commit_changes(layer, "line smoothing"):
                    return
                performed_commit = True
            elif not handle_edit_mode:
                # If we didn't manage edit mode, assume immediate apply
                performed_commit = True

            # Record history only after committed/applied
            if performed_commit:
                try:
                    if operation_type == 'copy' and self._created_feature_backup:
                        self.record_to_history(
                            description=f"Created smoothed copy of feature {feature.id()} in {layer.name()}",
                            undo_type='create_feature',
                            can_undo=True,
                            layers=[self.create_layer_descriptor(layer)],
                            features=[self._created_feature_backup]
                        )
                    elif operation_type == 'smooth' and self._feature_backup:
                        # Build payload expected by UpdateGeometryHandler:
                        # features: [{ 'fid': <fid>, 'old_geometry': {...}, 'new_geometry': {...} }]
                        try:
                            # Create new geometry backup for the updated feature
                            new_backup = self.create_feature_backup(feature_to_update, layer)
                            old_geom = self._feature_backup.get('geometry') if isinstance(self._feature_backup, dict) else None
                            new_geom = new_backup.get('geometry') if isinstance(new_backup, dict) else None

                            features_payload = [{
                                'fid': int(self._feature_backup.get('fid')) if isinstance(self._feature_backup, dict) and self._feature_backup.get('fid') is not None else feature_to_update.id(),
                                'old_geometry': old_geom,
                                'new_geometry': new_geom
                            }]

                            self.record_to_history(
                                description=f"Smoothed feature {feature.id()} in {layer.name()}",
                                undo_type='update_geometry',
                                can_undo=True,
                                layers=[self.create_layer_descriptor(layer)],
                            ...
                            # Fall back to legacy backup if something unexpected happens
                            try:
                                self.record_to_history(
                                    description=f"Smoothed feature {feature.id()} in {layer.name()}",
                                    undo_type='update_geometry',
                    ...
                    # Non-fatal: history recording shouldn't break user operation
                    pass
            
            # Show success message if enabled
            if show_success:
                if operation_type == "copy":
                    success_message = f"Smoothed copy of line feature ID {feature.id()} created successfully (ID: {feature_to_update.id()})"
                else:
                    success_message = f"Line feature ID {feature.id()} smoothed successfully"
                
                success_message += f"\n\nIterations: {iterations}\nOffset: {offset:.2f}"
                
                if show_line_length and line_length is not None:
                    try:
                        new_length = smoothed_geometry.length()
            ...
            self.show_error("Error", f"Failed to smooth line: {str(e)}")
