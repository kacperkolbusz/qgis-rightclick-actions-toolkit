"""
Copy Point Layer Style Action for Right-click Utilities and Shortcuts Hub

Allows copying the style (symbology, labeling, effects) from the clicked point layer
to other point layers in the project. Shows a dialog where users can select which
point layers to copy the style to.
"""
...
class CopyPointLayerStyleDialog(QDialog):
    """Dialog for selecting target point layers to copy style to."""
    ...
    def __init__(self, source_layer, target_layers, parent=None):
        ...
        self.setWindowTitle("Copy Point Layer Style")
    ...
    def init_ui(self):
        ...
        # Instructions
        instructions = QLabel("Select point layers to copy the style to:")
...
class CopyPointLayerStyleAction(BaseAction):
    """Action to copy style from one point layer to other point layers."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "copy_point_layer_style"
        self.name = "Copy Point Layer Style"
        self.category = "Styling"
        self.description = (
            "Copy the style (symbology, labeling, effects) from this point layer "
            "to other point layers in the project. A dialog will show all available "
            "point layers where you can select which ones to apply the style to."
        )
        self.enabled = True
    ...
    def execute(self, context):
        """Execute the copy point layer style action."""
        try:
        ...
        if not detected_features:
            self.show_error("Error", "No point layer found at this location")
            return
        ...
        if source_layer.geometryType() != QgsWkbTypes.PointGeometry:
            self.show_error("Error", "Source layer is not a point layer")
            return
        
        # Get all other point layers in the project
        target_layers = []
        ...
        if not target_layers:
            self.show_info("No Layers", "No other point layers found in the project")
            self.record_informational(
                description=f"Attempted to copy style from point layer '{source_layer.name()}' but no other point layers were available"
            )
            return
        
        # Show dialog to select target layers
        dialog = CopyPointLayerStyleDialog(source_layer, target_layers, None)
        ...
        try:
            ...
            if confirm_action:
                if errors:
                    ...
                    self.show_info(
                        "Copy Style Complete",
                        f"Successfully copied style from '{source_layer.name()}' to {copied_count} point layer(s)"
                    )
            
            # Record to history
            self.record_informational(
                description=f"Copied style from point layer '{source_layer.name()}' to {copied_count} point layer(s)",
                meta={
...
copy_point_layer_style_action = CopyPointLayerStyleAction()
